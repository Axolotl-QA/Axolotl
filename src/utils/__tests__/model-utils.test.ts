...
describe("isGemini3ModelFamily", () => {
	it("should return true for Gemini 3.x models", () => {
		isGemini3ModelFamily("gemini-3-pro-preview").should.equal(true);
		isGemini3ModelFamily("gemini-3-flash-preview").should.equal(true);
		isGemini3ModelFamily("gemini-3.1-pro-preview").should.equal(true);
		isGemini3ModelFamily("google/gemini-3-pro-preview").should.equal(true);
		isGemini3ModelFamily("google/gemini-3.1-pro-preview").should.equal(true);
	});

	it("should return false for non-Gemini-3 models", () => {
		isGemini3ModelFamily("gemini-2.5-pro").should.equal(false);
		isGemini3ModelFamily("gemini-pro").should.equal(false);
		isGemini3ModelFamily("gpt-5").should.equal(false);
	});
});
