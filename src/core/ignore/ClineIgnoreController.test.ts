...
describe("ClineIgnoreController", () => {
	...
	beforeEach(async () => {
		...
		// Create default .axolotlignore file
		await fs.writeFile(
			path.join(tempDir, ".axolotlignore"),
	...
	describe("Default Patterns", () => {
		...
		it("should block access to .axolotlignore file", async () => {
			const result = controller.validateAccess(".axolotlignore")
	...
	describe("Custom Patterns", () => {
		...
		it("should handle comments in .axolotlignore", async () => {
			// Create a new .axolotlignore with comments
			await fs.writeFile(
				path.join(tempDir, ".axolotlignore"),
	...
	describe("Error Handling", () => {
		...
		it("should handle missing .axolotlignore gracefully", async () => {
			// Create a new controller in a directory without .axolotlignore
		...
		it("should handle empty .axolotlignore", async () => {
			await fs.writeFile(path.join(tempDir, ".axolotlignore"), "")
	...
	describe("Include Directive", () => {
		it("should load patterns from an included file", async () => {
			...
			// Create a .axolotlignore file that includes .gitignore and adds an extra pattern "secret.txt"
			await fs.writeFile(path.join(tempDir, ".axolotlignore"), ["!include .gitignore", "secret.txt"].join("\n"))

			// Initialize the controller to load the updated .axolotlignore
			controller = new ClineIgnoreController(tempDir)
			await controller.initialize()
			...
			// "secret.txt" should be ignored as specified directly in .axolotlignore
		...
		it("should handle non-existent included file gracefully", async () => {
			// Create a .axolotlignore file that includes a non-existent file
			await fs.writeFile(path.join(tempDir, ".axolotlignore"), ["!include missing-file.txt"].join("\n"))
		...
		it("should handle non-existent included file gracefully alongside a valid pattern", async () => {
			...
			await fs.writeFile(path.join(tempDir, ".axolotlignore"), ["!include non-existent.txt", "*.tmp"].join("\n"))
