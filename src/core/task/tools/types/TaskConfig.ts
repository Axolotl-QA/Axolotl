...
import type { ComputerUseExecutor } from "@services/browser/ComputerUseExecutor";
...
export function validateTaskConfig(config: any): asserts config is TaskConfig {
	...
	if (config.services) {
		for (const key of TASK_SERVICES_KEYS) {
			if (OPTIONAL_TASK_SERVICES_KEYS.has(key)) {
				continue;
			}
	...
	if (config.callbacks) {
		for (const key of TASK_CALLBACKS_KEYS) {
			if (typeof config.callbacks[key] !== "function") {
				throw new Error(
					`Missing or invalid callbacks.${key} in TaskConfig (must be a function)`,
				);
			}
		}
	}
