...
export class AutoApprove {
	...
	private async getWorkspaceInfo(): Promise<{
		...
		if (
			this.workspacePathsCache === null ||
			this.isMultiRootScenarioCache === null
		) {
			...
			this.isMultiRootScenarioCache =
				isMultiRootEnabled(this.stateManager) &&
				this.workspacePathsCache.paths.length > 1;
		}
	...
	shouldAutoApproveTool(
		...
		if (this.stateManager.getGlobalSettingsKey("yoloModeToggled")) {
			switch (toolName) {
				...
				case ClineDefaultTool.COMPUTER_USE:
		...
		switch (toolName) {
			...
			case ClineDefaultTool.APPLY_PATCH:
				return [
					...
					autoApprovalSettings.actions.editFilesExternally ?? false,
				];
			case ClineDefaultTool.BASH:
				return [
	...
	async shouldAutoApproveToolWithPath(
		...
		const [autoApproveLocal, autoApproveExternal] = Array.isArray(
			autoApproveResult,
		)
			? autoApproveResult
