...
// API key resolution: env var YOU_API_KEY > hardcoded fallback
const YOU_API_URL = "https://api.you.com/v1/agents/runs"
const YOU_API_KEY =
	process.env.YOU_API_KEY || "ydc-sk-2311a7a444324459-cze0HaW3hpAy2soe44IqIcJdvjrsOD5R-e80a6ea1"
...
export class SentinelWebSearchHandler implements IToolHandler {
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		console.log("[SentinelWebSearch] Tool called with params:", JSON.stringify(block.params, null, 2))
		...
		if (!searchQuery) {
			...
			return await config.callbacks.sayAndCreateMissingParamError(this.name, "search_query")
		}

		config.taskState.consecutiveMistakeCount = 0

		try {
			console.log(`[SentinelWebSearch] Searching You.com API for: "${searchQuery}"`)
			...
			console.log(`[SentinelWebSearch] API response received, output items: ${data.output?.length || 0}`)
			...
			const terminalOutput = this.formatTerminalOutput(searchQuery, answerText, searchResults)
			...
			const escapedOutput = terminalOutput
				...
				.replace(/`/g, "\\`")
				.replace(/\n/g, "\\n")
			await config.callbacks.executeCommandTool(`printf "${escapedOutput}"`, 10)

			// Format results for LLM context (tool response)
			const llmOutput = this.formatLLMOutput(searchQuery, answerText, searchResults)
	...
	private formatTerminalOutput(query: string, answer: string, results: YouSearchResult[]): string {
		const lines: string[] = []
		lines.push("")
		lines.push("╔══════════════════════════════════════════════════════════════╗")
		...
		if (answer) {
			...
			const truncatedAnswer = answer.length > 1000 ? answer.substring(0, 1000) + "..." : answer
		...
		if (results.length > 0) {
			...
			for (const result of results.slice(0, 5)) {
				...
				if (result.snippet) {
					const shortSnippet =
						result.snippet.length > 150 ? result.snippet.substring(0, 150) + "..." : result.snippet
	...
	private formatLLMOutput(query: string, answer: string, results: YouSearchResult[]): string {
		const lines: string[] = []
		lines.push("=== SENTINEL WEB SEARCH RESULTS ===")
		lines.push(`Query: ${query}`)
		lines.push("")
		...
		lines.push("Use these search results to inform your test plan generation and code analysis.")
		return lines.join("\n")
	}
}
