...
export class AudioRecordingService {
	...
	async startRecording(): Promise<{ success: boolean; error?: string }> {
		try {
			...
			this.outputFile = path.join(tempDir, `cline_recording_${Date.now()}.ogg`);
	...
	getRecordingStatus(): {
		...
		const durationSeconds = this.isRecording
	...
	private checkRecordingDependencies(): { available: boolean; error?: string } {
		...
		if (!program) {
			...
			const error = config
				? config.error
				: `Audio recording is not supported on platform: ${platform}`;
	...
	private getRecordProgram():
		| { path: string; getArgs: (outputFile: string) => string[] }
		| undefined {
		const platform = os.platform() as keyof typeof AUDIO_PROGRAM_CONFIG;
		const config = AUDIO_PROGRAM_CONFIG[platform];
