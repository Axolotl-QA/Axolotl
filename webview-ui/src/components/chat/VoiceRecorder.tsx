...
const VoiceRecorder: React.FC<VoiceRecorderProps> = ({
	...
	const startRecording = useCallback(async () => {
		try {
			...
			const response = await DictationServiceClient.startRecording(
				EmptyRequest.create({}),
			);
	...
	const stopRecording = useCallback(async () => {
		try {
			...
			const response = await DictationServiceClient.stopRecording(
				EmptyRequest.create({}),
			);
			...
			const transcriptionResponse =
				await DictationServiceClient.transcribeAudio(
	...
	useEffect(() => {
		const pollRecordingStatus = async () => {
			try {
				const statusResponse = await DictationServiceClient.getRecordingStatus(
					EmptyRequest.create({}),
				);
	...
	const cancelRecording = useCallback(async () => {
		try {
			...
			const response = await DictationServiceClient.cancelRecording(
				EmptyRequest.create({}),
			);
	...
	const iconAdjustment =
		isProcessing || isStarting ? "mt-0" : error ? "mt-1" : "mt-0.5";
		...
		return (
			<Tooltip>
				<TooltipTrigger asChild>
					<div
						className={cn(
							"pt-1 input-icon-button mr-1.5 text-base",
							iconAdjustment,
							{
		...
		>
			<Tooltip>
				<TooltipTrigger asChild>
					<Button
						className={cn(
							"input-icon-button p-1 m-0 mr-1.5 text-base",
							iconAdjustment,
							{
			...
			<Tooltip>
				<TooltipTrigger asChild>
					<Button
						className={cn(
							"input-icon-button p-1 m-0 text-base text-error",
							iconAdjustment,
							{
								"animate-spin": isProcessing || isStarting,
								disabled: disabled || isProcessing,
