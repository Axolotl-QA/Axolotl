...
async function insforgeDbQuery(table, filters) {
	...
	const res = await fetch(
		`${INSFORGE_BASE_URL}/api/database/records/${table}?${params}`,
		{
	...
	if (!res.ok)
		return {
			data: null,
			error: await res.json().catch(() => ({ message: "DB query failed" })),
		};
...
async function insforgeDbInsert(table, records) {
	const res = await fetch(
		`${INSFORGE_BASE_URL}/api/database/records/${table}`,
		{
			method: "POST",
			headers: {
	...
	if (!res.ok)
		return {
			data: null,
			error: await res.json().catch(() => ({ message: "DB insert failed" })),
		};
	const data = await res.json();
	return { data, error: null };
}
...
async function insforgeDbUpsert(table, records) {
	...
	if (!res.ok)
		return {
			data: null,
			error: await res.json().catch(() => ({ message: "DB upsert failed" })),
		};
	const data = await res.json();
	return { data, error: null };
}
...
async function insforgeDbUpdate(table, filters, updates) {
	...
	if (!res.ok)
		return {
			error: await res.json().catch(() => ({ message: "DB update failed" })),
		};
	return { error: null };
}
...
async function insforgeGetUser(accessToken) {
	...
	if (!res.ok)
		return {
			user: null,
			error: await res.json().catch(() => ({ message: "Invalid token" })),
		};
	const data = await res.json();
...
fastify.get("/v1/auth/authorize", async (request, reply) => {
	...
	if (!redirect_uri || !state || !code_challenge) {
		return reply
			.code(400)
			.send({ error: "Missing redirect_uri/state/code_challenge" });
	}
...
fastify.post("/v1/auth/code", async (request, reply) => {
	...
	const { data: authRequests, error: authReqError } = await insforgeDbQuery(
		"auth_requests",
		{
			state: `eq.${state}`,
...
fastify.post("/v1/auth/token", async (request, reply) => {
	...
	if (!code || !code_verifier || !redirect_uri) {
		return reply
			.code(400)
			.send({ error: "Missing code/code_verifier/redirect_uri" });
	}

	const codeHash = hashSha256(code);
	const { data: codeRows, error: codeError } = await insforgeDbQuery(
		"auth_codes",
		{
			code_hash: `eq.${codeHash}`,
			select:
				"code_hash,user_id,state,access_token,refresh_token,expires_at,used_at",
			limit: "1",
		},
	);
...
fastify.post("/v1/auth/refresh", async (request, reply) => {
	...
	const res = await fetch(
		`${INSFORGE_BASE_URL}/api/auth/refresh?client_type=desktop`,
		{
			method: "POST",
	...
	if (!res.ok) {
		const err = await res
			.json()
			.catch(() => ({ message: "Failed to refresh session" }));
		return reply
			.code(401)
			.send({ error: err.message || "Failed to refresh session" });
	}
...
fastify.get("/api/v1/users/:id/balance", async (request, reply) => {
	const token = extractToken(request);
	if (!token) return reply.code(401).send({ error: "Missing token" });

	const { user, error } = await insforgeGetUser(token);
	if (error || !user || user.id !== request.params.id) {
		return reply.code(401).send({ error: "Unauthorized" });
	}

	const { data } = await insforgeDbQuery("user_credits", {
		user_id: `eq.${user.id}`,
	...
	return reply.send({
		success: true,
		data: { balance: { currentBalance: balanceCents }, userId: user.id },
		usageTransactions: [],
		paymentTransactions: [],
	});
});
...
fastify.get("/api/v1/users/:id/usages", async (request, reply) => {
	const token = extractToken(request);
	if (!token) return reply.code(401).send({ error: "Missing token" });
	...
	const { data } = await insforgeDbQuery("api_request_logs", {
		user_id: `eq.${user.id}`,
		select: "*",
...
// Get user payment history (stub)
fastify.get("/api/v1/users/:id/payments", async (request, reply) => {
	return reply.send({ success: true, data: { paymentTransactions: [] } });
});
